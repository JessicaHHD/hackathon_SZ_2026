...
4. image_prompt 必须包含 visual_anchors 内容，并在开头加上：same character throughout, consistent appearance, 结尾加：--v 6.0, flat illustration, vector art, high quality, soft lighting
    ...
    for attempt in range(retries):
        try:
            ...
            if res.status_code != 200:
                err_body = res.text
                print(f"[generate_image] HTTP {res.status_code}: {err_body}")
                if "safety" in err_body.lower() or "rejected" in err_body.lower():
            ...
            for _ in range(21):
                ...
                if result["status"] == "failed":
                    fail_reason = str(result.get("fail_reason") or result.get("message") or "")
                    print(f"[generate_image] failed (attempt {attempt}): {fail_reason}")
                    current_prompt = rewrite_page(current_prompt) if attempt == 0 else rewrite_prompt(current_prompt)
                    break
                time.sleep(5)
